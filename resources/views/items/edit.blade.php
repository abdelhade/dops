...
@section('title', __('dobs.edit_item'))

@section('header_title', __('dobs.edit_item'))
@section('header_subtitle', __('dobs.edit_item_subtitle'))
    ...
    <form action="{{ route('items.update', $item->id) }}" method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control" placeholder="{{ __('dobs.item_name_placeholder') }}" value="{{ old('name', $item->name) }}" required>
            </div>

            <div class="form-group">
                <label for="sku" class="form-label">{{ __('dobs.sku') }}</label>
                <input type="text" name="sku" id="sku" class="form-control" placeholder="{{ __('dobs.sku_placeholder') }}" value="{{ old('sku', $item->sku) }}">
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <select name="paper_size_id" id="paper_size_id" class="form-control">
                    ...
                    @foreach($paperSizes as $ps)
                        <option value="{{ $ps->id }}" {{ old('paper_size_id', $item->paper_size_id) == $ps->id ? 'selected' : '' }}>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="price" id="price" class="form-control" placeholder="{{ __('dobs.zero_decimal') }}" value="{{ old('price', $item->price) }}" required>
            </div>

            <div class="form-group">
                <label for="stock" class="form-label">{{ __('dobs.stock_level') }} <span style="color: var(--color-danger)">{{ __('dobs.required_mark') }}</span></label>
                <input type="number" name="stock" id="stock" class="form-control" placeholder="0" value="{{ old('stock', $item->stock) }}" required>
            </div>
        </div>

        <div class="form-group">
            <label for="description" class="form-label">{{ __('dobs.item_description') }}</label>
            <textarea name="description" id="description" class="form-control" placeholder="{{ __('dobs.item_description_placeholder') }}">{{ old('description', $item->description) }}</textarea>
        </div>

        <div class="form-actions">
            <a href="{{ route('items.index') }}" class="btn btn-secondary">{{ __('dobs.cancel') }}</a>
            <button type="submit" class="btn btn-primary">
                <i class="fa-solid fa-save"></i> {{ __('dobs.update_item') }}
            </button>
        </div>
    </form>
</div>
