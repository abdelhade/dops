...
@section('title', __('dobs.activity_details'))

@section('header_title', $activity->name)
@section('header_subtitle', __('dobs.activity_details_subtitle'))
...
<div style="max-width: 800px; margin: 0 auto;">
    <div class="glass-card">
        <h2 class="card-title" style="margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
            ...
            {{ __('dobs.activity_details') }}
        </h2>
        
        <div style="display: grid; grid-template-columns: 1fr; gap: 1.5rem;">
            <div>
                <span class="stat-label">{{ __('dobs.activity_name') }}</span>
                <div style="font-size:1.15rem; font-weight:600; margin-top:0.25rem;">{{ $activity->name }}</div>
            </div>
        </div>
        ...
        <div style="margin-top: 2rem; border-top: 1px solid var(--border-color); padding-top: 1.5rem;">
            ...
            <div style="font-size:1rem; color:var(--text-secondary); margin-top:0.5rem; white-space:pre-line;">
                {{ $activity->description ?: __('dobs.no_description_provided') }}
            </div>
        </div>
    </div>
</div>
