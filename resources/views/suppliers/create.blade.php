...
@section('title', __('dobs.create_supplier'))

@section('header_title', __('dobs.create_supplier'))
@section('header_subtitle', __('dobs.create_supplier_subtitle'))
...
<div class="glass-card" style="max-width: 600px; margin: 0 auto;">
    <form action="{{ route('suppliers.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control" placeholder="{{ __('dobs.supplier_name_placeholder') }}" value="{{ old('name') }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="address" id="address" class="form-control" placeholder="{{ __('dobs.business_address_placeholder') }}">{{ old('address') }}</textarea>
        </div>

        <div class="form-actions">
            <a href="{{ route('suppliers.index') }}" class="btn btn-secondary">{{ __('dobs.cancel') }}</a>
            <button type="submit" class="btn btn-primary">
                <i class="fa-solid fa-save"></i> {{ __('dobs.save_supplier') }}
            </button>
        </div>
    </form>
</div>
